...
const CHAIR_R   = 7;
const CHAIR_GAP = 6;
// Extra padding so chairs sitting on the room boundary are never clipped
const PAD       = 72;
...
function FloorPlan({ tables, zones, room, bookedIds, selectedId, primaryColor, onSelect }) {
  ...
  if (!tables.length) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center px-6 rounded-2xl border"
        style={{ background: '#faf7f2', borderColor: 'rgba(200,160,80,0.2)' }}>
        <Crown size={28} className="mb-3" style={{ color: primaryColor, opacity: 0.35 }}/>
        <p className="text-sm font-bold text-stone-400 mb-1">No tables configured</p>
  ...
  // viewBox: room bounds + generous PAD so edge chairs are always fully visible
  let vx, vy, vw, vh;
  if (room) {
    vx = room.x - PAD;
    vy = room.y - PAD;
  ...
  return (
    <div
      ref={containerRef}
      className="relative w-full rounded-2xl overflow-hidden select-none border"
      style={{ borderColor: 'rgba(200,160,80,0.25)' }}
    >
      {/*
        SVG fills 100% width, height is determined automatically by the viewBox
        aspect ratio — no maxHeight cap that would distort or clip the plan.
      */}
      <svg
        viewBox={`${vx} ${vy} ${vw} ${vh}`}
      ...
      >
        <defs>
          {/* Warm linen background outside the room */}
          <pattern id="fp-outer" width="1" height="1" patternUnits="objectBoundingBox">
            <rect width="1" height="1" fill="#f0ebe0"/>
          </pattern>

          {/* Light wood plank floor inside the room */}
          <pattern id="fp-floor" width="90" height="18" patternUnits="userSpaceOnUse">
            <rect width="90" height="18" fill="#f5e7cc"/>
            <rect y="1" width="90" height="16" fill="#f8edd4"/>
            <line x1="0" y1="0"  x2="90" y2="0"  stroke="#e0ccaa" strokeWidth="1"/>
            <line x1="10" y1="6" x2="70" y2="6"  stroke="rgba(160,110,50,0.06)" strokeWidth="0.6"/>
            <line x1="28" y1="13" x2="82" y2="13" stroke="rgba(160,110,50,0.04)" strokeWidth="0.5"/>
          </pattern>

          {/* Subtle dot grid overlay */}
          <pattern id="fp-dots" width="20" height="20" patternUnits="userSpaceOnUse">
            <circle cx="10" cy="10" r="0.7" fill="rgba(150,110,50,0.14)"/>
          </pattern>

          {/* Table wood radial gradients */}
          {tables.map(t => (
            <radialGradient key={t._id} id={`fp-tbl-${t._id}`} cx="38%" cy="30%" r="70%" gradientUnits="objectBoundingBox">
          ...
          {/* Selected table glow */}
          <filter id="fp-glow-sel" x="-50%" y="-50%" width="200%" height="200%">
          ...
          {/* Hover table glow */}
          <filter id="fp-glow-hov" x="-40%" y="-40%" width="180%" height="180%">
            <feGaussianBlur stdDeviation="4" result="blur"/>
            <feFlood floodColor="#c8860a" floodOpacity="0.4" result="color"/>
            <feComposite in="color" in2="blur" operator="in" result="glow"/>
            <feMerge><feMergeNode in="glow"/><feMergeNode in="SourceGraphic"/></feMerge>
          </filter>
        </defs>
        ...
        {/* Outer area (linen) */}
        <rect x={vx} y={vy} width={vw} height={vh} fill="#ede8de"/>

        {/* Room floor */}
        {room ? (
          <>
            <rect x={room.x} y={room.y} width={room.w} height={room.h} rx={10} fill="url(#fp-floor)"/>
            <rect x={room.x} y={room.y} width={room.w} height={room.h} rx={10}
              fill="url(#fp-dots)" style={{ pointerEvents: 'none' }}/>
            <rect x={room.x} y={room.y} width={room.w} height={room.h} rx={10}
              fill="none" stroke="rgba(160,120,50,0.3)" strokeWidth="1.5"
              style={{ pointerEvents: 'none' }}/>
          </>
        ) : (
          /* No room defined — plain floor fill under tables */
          <rect x={vx} y={vy} width={vw} height={vh} fill="url(#fp-floor)"/>
        )}
        ...
        {zones?.map(z => (
          <g key={z.id} style={{ pointerEvents: 'none' }}>
            <rect x={z.x} y={z.y} width={z.w} height={z.h} rx={6}
              ...
              stroke={z.color ?? '#f59e0b'} strokeOpacity={0.5}
              strokeWidth="1.2" strokeDasharray="5 3"/>
            {z.label && (
              <text x={z.x + 10} y={z.y + 16} fontSize={10} fontWeight="700"
                fill={z.color ?? '#f59e0b'}
        ...
        {tables.map(t => {
          ...
          const filt     = isSel ? 'url(#fp-glow-sel)' : isHov ? 'url(#fp-glow-hov)' : undefined;
          const tblFill  = isBooked ? '#c8c0b8' : isSel ? primaryColor : `url(#fp-tbl-${t._id})`;
          const tblStroke = isSel ? primaryColor : isBooked ? '#a8a09a' : '#8a5e22';
          const chairFill   = isBooked ? '#d4cec8' : '#e8d5b0';
          const chairStroke = isBooked ? '#b0a89e' : 'rgba(140,90,20,0.5)';
          const numFill  = isBooked ? '#a09890' : isSel ? '#fff' : '#2a1a06';
          const capFill  = isBooked ? '#b0a89e' : isSel ? 'rgba(255,255,255,0.7)' : 'rgba(60,35,8,0.45)';
          ...
          return (
            ...
            >
              ...
              {t.shape === 'round' ? (
                <g filter={filt}>
                  <circle cx={tx} cy={ty} r={ROUND_R}
                    fill={tblFill} stroke={tblStroke} strokeWidth={isSel ? 2.5 : 1.5}/>
                  <circle cx={tx} cy={ty} r={ROUND_R * 0.6}
                    fill={isSel ? 'rgba(255,255,255,0.2)' : 'rgba(255,248,235,0.25)'}
                    style={{ pointerEvents: 'none' }}/>
                </g>
              ) : t.shape === 'rectangle' ? (
                <g filter={filt}>
                  <rect x={tx-hw} y={ty-hh} width={hw*2} height={hh*2} rx={5}
                    fill={tblFill} stroke={tblStroke} strokeWidth={isSel ? 2.5 : 1.5}/>
                  <rect x={tx-hw*0.6} y={ty-hh*0.6} width={hw*1.2} height={hh*1.2} rx={3}
                    fill={isSel ? 'rgba(255,255,255,0.2)' : 'rgba(255,248,235,0.25)'}
                    style={{ pointerEvents: 'none' }}/>
                </g>
              ) : (
                <g filter={filt}>
                  <rect x={tx-hw} y={ty-hh} width={hw*2} height={hh*2} rx={7}
                    fill={tblFill} stroke={tblStroke} strokeWidth={isSel ? 2.5 : 1.5}/>
                  <rect x={tx-hw*0.6} y={ty-hh*0.6} width={hw*1.2} height={hh*1.2} rx={5}
                    fill={isSel ? 'rgba(255,255,255,0.2)' : 'rgba(255,248,235,0.25)'}
                    style={{ pointerEvents: 'none' }}/>
                </g>
              )}
              ...
              <text x={tx} y={ty - 4} textAnchor="middle"
                fill={numFill} fontSize={11} fontWeight="800"
                style={{ userSelect: 'none', pointerEvents: 'none' }}>
                {t.number}
              </text>
              <text x={tx} y={ty + 10} textAnchor="middle"
                fill={capFill} fontSize={8.5}
                style={{ userSelect: 'none', pointerEvents: 'none' }}>
                {t.capacity}p
              </text>
            </g>
      ...
      {tooltip && (
        <div className="absolute z-20 pointer-events-none"
          ...
          <div className="rounded-xl px-3.5 py-2.5 text-left min-w-[140px] shadow-xl"
            style={{ background: '#fff', border: '1px solid rgba(200,160,70,0.25)' }}>
            ...
            <div className="absolute left-1/2 -translate-x-1/2 top-full w-3 h-2 overflow-hidden">
              <div className="w-3 h-3 bg-white rotate-45 -translate-y-1.5"
                style={{ border: '1px solid rgba(200,160,70,0.25)' }}/>
            </div>
          </div>
        </div>
      )}
      ...
      {/* Legend */}
      <div className="absolute bottom-2.5 right-3 flex gap-3 text-[9px] font-semibold"
        style={{ color: 'rgba(100,70,20,0.5)' }}>
        <span className="flex items-center gap-1.5">
          <span className="w-2.5 h-2.5 rounded-full" style={{ background: '#a06830' }}/>Available
        </span>
        <span className="flex items-center gap-1.5">
          <span className="w-2.5 h-2.5 rounded-full" style={{ background: primaryColor }}/>Selected
        </span>
        <span className="flex items-center gap-1.5">
          <span className="w-2.5 h-2.5 rounded-full bg-stone-300"/>Booked
        </span>
      </div>
    </div>
  );
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4"
      ...
      <div
        ...
        style={{ background: '#fff', border: '1px solid rgba(220,200,160,0.35)' }}
        onClick={e => e.stopPropagation()}
      >
        {/* Header */}
        <div className="sticky top-0 z-10 flex items-center justify-between px-5 sm:px-6 py-4"
          style={{ background: 'rgba(255,255,255,0.97)', backdropFilter: 'blur(16px)', borderBottom: '1px solid rgba(220,200,160,0.25)' }}>
        ...
        {step === 2 ? (
          ...
          <div className="grid lg:grid-cols-[1fr_340px]">
            ...
            <div className="p-5 border-b lg:border-b-0 lg:border-r"
              style={{ borderColor: 'rgba(220,200,160,0.25)' }}>
                ...
                <div>
                  ...
                  {selectedTable ? (
                    ...
                    <p className="text-sm text-gray-400">
                      {floorLoading ? 'Loading floor plan…' : 'Click an available table to select it'}
                    </p>
                  )}
                </div>
                {selectedTableId && (
                  <button onClick={() => setSelectedTableId(null)}
                    className="text-[10px] text-gray-400 hover:text-gray-700 border border-gray-200 rounded-lg px-2.5 py-1 hover:border-gray-400 transition-colors">
                    Clear
                  </button>
                )}
              </div>
              ...
              {floorLoading ? (
                <div className="flex items-center justify-center rounded-2xl border"
                  style={{ minHeight: 260, background: '#faf7f2', borderColor: 'rgba(200,160,80,0.2)' }}>
              ...
              {!form.date && tables.length > 0 && (
                <p className="mt-3 text-center text-[11px] font-medium"
            ...
            <div className="flex flex-col p-5 gap-5 overflow-y-auto" style={{ maxHeight: '80vh' }}>
              ...
              {/* Date */}
              <FormField label="Date" icon={<Calendar size={12} style={{ color: primaryColor }}/>}>
                <input
                  type="date"
              ...
              {/* Time — lunch / dinner groups */}
              <FormField label="Time" icon={<Clock size={12} style={{ color: primaryColor }}/>}>
                <div className="space-y-2">
                  <p className="text-[9px] font-black uppercase tracking-widest text-gray-300">Lunch</p>
                  <div className="grid grid-cols-3 gap-1.5">
                    {LUNCH.map(t => (
                      <TimeChip key={t} time={t} selected={form.time === t}
                        primary={primaryColor} onSelect={() => setF('time', t)}/>
                    ))}
                  </div>
              ...
              {/* Party size */}
              <FormField label="Guests" icon={<Users size={12} style={{ color: primaryColor }}/>}>
                <div className="flex items-center gap-3">
                  <button onClick={() => setF('partySize', Math.max(1, form.partySize - 1))}
                    ...
                    style={{ background: '#faf6ef', border: '1px solid rgba(200,160,80,0.25)' }}>
                    −
                  </button>
              ...
              {/* Contact info */}
              <FormField label="Your Name" icon={<User size={12} style={{ color: primaryColor }}/>}>
                <LightInput type="text" placeholder="Full name" value={form.customerName}
                  onChange={e => setF('customerName', e.target.value)}/>
              </FormField>

              <FormField label="Phone" icon={<Phone size={12} style={{ color: primaryColor }}/>}>
                <LightInput type="tel" placeholder="+216 XX XXX XXX" value={form.customerPhone}
                  onChange={e => setF('customerPhone', e.target.value)}/>
              </FormField>

              <FormField label="Email (optional)" icon={<Mail size={12} style={{ color: primaryColor }}/>}>
                <LightInput type="email" placeholder="your@email.com" value={form.customerEmail}
                  onChange={e => setF('customerEmail', e.target.value)}/>
              </FormField>

              <FormField label="Special Requests" icon={<FileText size={12} style={{ color: primaryColor }}/>}>
                <textarea
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 py-3.5 rounded-2xl text-sm font-black text-white transition-all disabled:opacity-40 disabled:cursor-not-allowed hover:scale-[1.02]"
                style={{
                  backgroundColor: canSubmit ? primaryColor : '#c8b890',
              ...
              <p className="text-[10px] text-amber-900/30 text-center leading-relaxed -mt-2">
...
function LightInput({ type, placeholder, value, onChange }) {
  return (
    <input
      type={type}
...
function TimeChip({ time, selected, primary, onSelect }) {
  return (
    <button
      ...
      style={selected
        ? { background: primary, color: '#fff', boxShadow: `0 4px 12px ${primary}40` }
        : { background: '#faf6ef', color: '#7a4f1a', border: '1px solid rgba(200,160,80,0.25)' }
      }
    >
      {time}
    </button>
